...
export default function AdminDashboard() {
  ...
  return (
    <div className="flex flex-col gap-8 w-full p-2 md:p-8">
      <h1 className="text-2xl font-bold text-black">Welcome to the Admin Dashboard</h1>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
        <div className="bg-white rounded-xl shadow p-6 flex flex-col items-center">
          <span className="text-3xl font-bold text-black">
            {productCount === null ? '...' : productCount}
          </span>
          <span className="text-black mt-2 font-bold">Products</span>
        </div>
        <div className="bg-white rounded-xl shadow p-6 flex flex-col items-center">
          <span className="text-3xl font-bold text-black">
            {orderCount === null ? '...' : orderCount}
          </span>
          <span className="text-black mt-2 font-bold">Orders</span>
        </div>
        <div className="bg-white rounded-xl shadow p-6 flex flex-col items-center">
          <span className="text-3xl font-bold text-black">
            {reviewCount === null ? '...' : reviewCount}
          </span>
          <span className="text-black mt-2 font-bold">Customer Reviews</span>
        </div>
      </div>
    </div>
  );
